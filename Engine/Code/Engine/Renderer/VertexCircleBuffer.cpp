...
VertexCircleBuffer::VertexCircleBuffer(const RHIDevice& owner, const buffer_t& buffer, const BufferUsage& usage, const BufferBindUsage& bindUsage) noexcept
: ArrayBuffer<VertexCircle2D>() {
    ...
    GUARANTEE_OR_DIE(SUCCEEDED(hr), "VertexCircleBuffer failed to create.");
}

VertexCircleBuffer::~VertexCircleBuffer() noexcept {
    if(IsValid()) {
        m_dx_buffer.Reset();
        m_dx_buffer = nullptr;
    }
...
void VertexCircleBuffer::Update(RHIDeviceContext& context, const buffer_t& buffer) noexcept {
