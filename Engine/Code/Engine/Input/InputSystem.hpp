...
private:
    ...
    std::bitset<(std::size_t)KeyCode::Max> m_previousKeys{};
    std::bitset<(std::size_t)KeyCode::Max> m_currentKeys{};
    Vector2 m_mouseCoords = Vector2::Zero;
    Vector2 m_mousePrevCoords = Vector2::Zero;
    Vector2 m_mouseDelta = Vector2::Zero;
    mutable AABB2 m_initialClippingArea{};
    mutable AABB2 m_currentClippingArea{};
    Stopwatch m_connection_poll = Stopwatch(TimeUtils::FPSeconds{1.0f});
    int m_mouseWheelPosition = 0;
    int m_mouseWheelHPosition = 0;
    const int m_max_controller_count = 4;
    bool m_cursor_visible = true;
    mutable bool m_should_clip_cursor = false;
};
